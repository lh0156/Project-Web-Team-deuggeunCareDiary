...
@WebServlet("/admin/membermanagement/memberOrderDetail.do")
...
//		get방식으로 memberseq 받아오기
		...
		ArrayList<ManagementOrderDTO> subDto = dao.getUserOrder(memberseq);		//서브DTO
		
		
		req.setAttribute("memberseq", memberseq);
		req.setAttribute("mainDto", mainDto);
		req.setAttribute("subDto", subDto);
		RequestDispatcher dispatcher = req.getRequestDispatcher("/WEB-INF/views/admin/membermanagement/memberOrderDetail.jsp");
